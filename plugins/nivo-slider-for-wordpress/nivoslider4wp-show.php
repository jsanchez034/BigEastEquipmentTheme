<?php
	...
	<div id="slider">
				...
				<?php foreach($items as $item) : ?>
						...
						<img src="<?php echo $ns4wp_filesurl.$item->nivoslider4wp_id.'_s.'.$item->nivoslider4wp_type; ?>" alt="<?php echo stripslashes(_e($item->nivoslider4wp_text_headline)); ?>" title="<?php echo stripslashes(_e($item->nivoslider4wp_text_headline)); ?>"/>
						<?php } else { 
							$link = $item->nivoslider4wp_image_link;
							if (strpos($link,"?")) { 
								$link = $link . "&lang=" .  qtrans_getLanguage();
							} else {
								$link = $link . "/?lang=" .  qtrans_getLanguage();
							}
							 ?>
						<a href="<?php echo $link;?>"><img src="<?php echo $ns4wp_filesurl.$item->nivoslider4wp_id.'_s.'.$item->nivoslider4wp_type; ?>" alt="<?php echo stripslashes(_e($item->nivoslider4wp_text_headline)); ?>" title="<?php echo stripslashes(_e($item->nivoslider4wp_text_headline)); ?>"/></a>
						<?php } ?>
				<?php endforeach; ?>
		</div>
	<?php
